...
describe('Payment e2e test', () => {
  ...
  const paymentSample = { amount: 15393.84, paymentDate: '2025-02-12T23:38:44.583Z', paymentMethod: 'TRANSFER' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {
        startDate: '2025-02-12T19:33:45.519Z',
        endDate: '2025-02-12T05:51:32.022Z',
        monthlyRent: 24731.08,
        securityDeposit: 931.05,
        status: 'CANCELED',
      },
    }).then(({ body }) => {
      rentalContract = body;
  ...
  describe('new Payment page', () => {
    ...
    it('should create an instance of Payment', () => {
      cy.get(`[data-cy="amount"]`).type('6867.8');
      cy.get(`[data-cy="amount"]`).should('have.value', '6867.8');

      cy.get(`[data-cy="paymentDate"]`).type('2025-02-12T16:54');
      cy.get(`[data-cy="paymentDate"]`).blur();
      cy.get(`[data-cy="paymentDate"]`).should('have.value', '2025-02-12T16:54');

      cy.get(`[data-cy="paymentMethod"]`).select('CARD');

      cy.get(`[data-cy="reference"]`).type('equally even');
      cy.get(`[data-cy="reference"]`).should('have.value', 'equally even');

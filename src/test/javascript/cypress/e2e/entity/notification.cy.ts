...
describe('Notification e2e test', () => {
  ...
  const notificationSample = {
    target: 'AD',
    content: 'breastplate beautifully whenever',
    status: 'FAILED',
    timestamp: '2025-02-12T17:42:20.794Z',
  };
  ...
  describe('new Notification page', () => {
    ...
    it('should create an instance of Notification', () => {
      cy.get(`[data-cy="target"]`).select('THANKS');

      cy.get(`[data-cy="content"]`).type('serene uh-huh furiously');
      cy.get(`[data-cy="content"]`).should('have.value', 'serene uh-huh furiously');

      cy.get(`[data-cy="status"]`).select('VIEWED');

      cy.get(`[data-cy="timestamp"]`).type('2025-02-12T06:37');
      cy.get(`[data-cy="timestamp"]`).blur();
      cy.get(`[data-cy="timestamp"]`).should('have.value', '2025-02-12T06:37');

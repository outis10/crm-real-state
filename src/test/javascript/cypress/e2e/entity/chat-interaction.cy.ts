...
describe('ChatInteraction e2e test', () => {
  ...
  const chatInteractionSample = {
    customerMessage: 'finally amid once',
    chatbotResponse: 'camouflage',
    timestamp: '2025-02-12T02:33:38.661Z',
  };
  ...
  describe('new ChatInteraction page', () => {
    ...
    it('should create an instance of ChatInteraction', () => {
      cy.get(`[data-cy="customerMessage"]`).type('progress');
      cy.get(`[data-cy="customerMessage"]`).should('have.value', 'progress');

      cy.get(`[data-cy="chatbotResponse"]`).type('siege');
      cy.get(`[data-cy="chatbotResponse"]`).should('have.value', 'siege');

      cy.get(`[data-cy="timestamp"]`).type('2025-02-12T09:15');
      cy.get(`[data-cy="timestamp"]`).blur();
      cy.get(`[data-cy="timestamp"]`).should('have.value', '2025-02-12T09:15');

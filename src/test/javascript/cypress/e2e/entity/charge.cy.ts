...
describe('Charge e2e test', () => {
  ...
  const chargeSample = { type: 'OTHER', amount: 27523.3, dueDate: '2025-02-12T12:02:35.450Z', status: 'OVERDUE' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {
        startDate: '2025-02-12T23:46:11.298Z',
        endDate: '2025-02-12T19:14:43.576Z',
        monthlyRent: 8966.03,
        securityDeposit: 22833.99,
        status: 'FINISHED',
      },
    }).then(({ body }) => {
      rentalContract = body;
  ...
  describe('new Charge page', () => {
    ...
    it('should create an instance of Charge', () => {
      cy.get(`[data-cy="type"]`).select('LEASE');

      cy.get(`[data-cy="amount"]`).type('19176.72');
      cy.get(`[data-cy="amount"]`).should('have.value', '19176.72');

      cy.get(`[data-cy="dueDate"]`).type('2025-02-12T09:00');
      cy.get(`[data-cy="dueDate"]`).blur();
      cy.get(`[data-cy="dueDate"]`).should('have.value', '2025-02-12T09:00');

      cy.get(`[data-cy="status"]`).select('OVERDUE');

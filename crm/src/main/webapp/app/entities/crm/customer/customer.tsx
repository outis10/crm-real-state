...
export const Customer = () => {
  ...
  const customerList = useAppSelector(state => state.crm.customer.entities);
  const loading = useAppSelector(state => state.crm.customer.loading);
  const totalItems = useAppSelector(state => state.crm.customer.totalItems);
  ...
  return (
    <div>
      <h2 id="customer-heading" data-cy="CustomerHeading">
        <Translate contentKey="crmApp.crmCustomer.home.title">Customers</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="crmApp.crmCustomer.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/crm/customer/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="crmApp.crmCustomer.home.createLabel">Create new Customer</Translate>
          </Link>
        </div>
      </h2>
        ...
        {customerList && customerList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="crmApp.crmCustomer.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('city')}>
                  <Translate contentKey="crmApp.crmCustomer.city">City</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('city')} />
                </th>
                ...
                <th className="hand" onClick={sort('rentalBudget')}>
                  <Translate contentKey="crmApp.crmCustomer.rentalBudget">Rental Budget</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('rentalBudget')} />
                </th>
                <th className="hand" onClick={sort('interactionHistory')}>
                  <Translate contentKey="crmApp.crmCustomer.interactionHistory">Interaction History</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('interactionHistory')} />
                </th>
            ...
            <tbody>
              {customerList.map((customer, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/crm/customer/${customer.id}`} color="link" size="sm">
                      {customer.id}
                    </Button>
                  </td>
                  ...
                  <td>{customer.socialMediaProfiles}</td>
                  <td>{customer.notes}</td>
                  <td>{customer.preferences}</td>
                  <td>{customer.budget}</td>
                  <td>{customer.rentalBudget}</td>
                  <td>{customer.interactionHistory}</td>
                  <td>{customer.createdBy}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/crm/customer/${customer.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/crm/customer/${customer.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/crm/customer/${customer.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="crmApp.crmCustomer.home.notFound">No Customers found</Translate>
            </div>
          )
        )}
      </div>

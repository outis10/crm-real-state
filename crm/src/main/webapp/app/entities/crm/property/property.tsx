...
export const Property = () => {
  ...
  const propertyList = useAppSelector(state => state.crm.property.entities);
  const loading = useAppSelector(state => state.crm.property.loading);
  const links = useAppSelector(state => state.crm.property.links);
  ...
  return (
    <div>
      <h2 id="property-heading" data-cy="PropertyHeading">
        <Translate contentKey="crmApp.crmProperty.home.title">Properties</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="crmApp.crmProperty.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/crm/property/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="crmApp.crmProperty.home.createLabel">Create new Property</Translate>
          </Link>
        </div>
      </h2>
          ...
          {propertyList && propertyList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="crmApp.crmProperty.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
              ...
              <tbody>
                {propertyList.map((property, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    <td>
                      <Button tag={Link} to={`/crm/property/${property.id}`} color="link" size="sm">
                        {property.id}
                      </Button>
                    </td>
                    <td>{property.name}</td>
                    ...
                    <td>{property.type}</td>
                    <td>
                      <Translate contentKey={`crmApp.OperationTypeEnum.${property.operationType}`} />
                    </td>
                    ...
                    <td>
                      <Translate contentKey={`crmApp.PropertyStatusEnum.${property.status}`} />
                    </td>
                    <td>{property.images}</td>
                    <td>{property.createdBy}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`/crm/property/${property.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                          <FontAwesomeIcon icon="eye" />{' '}
                          <span className="d-none d-md-inline">
                            <Translate contentKey="entity.action.view">View</Translate>
                          </span>
                        </Button>
                        <Button tag={Link} to={`/crm/property/${property.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        ...
                        <Button
                          onClick={() => (window.location.href = `/crm/property/${property.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="crmApp.crmProperty.home.notFound">No Properties found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>

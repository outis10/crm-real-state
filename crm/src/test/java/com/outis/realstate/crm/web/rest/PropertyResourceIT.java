...
import static com.outis.realstate.crm.domain.PropertyAsserts.*;
...
import com.outis.realstate.crm.repository.PropertyRepository;
import com.outis.realstate.crm.service.dto.PropertyDTO;
import com.outis.realstate.crm.service.mapper.PropertyMapper;
...
class PropertyResourceIT {
    ...
    void createProperty() throws Exception {
        ...
        var returnedPropertyDTO = om.readValue(
            restPropertyMockMvc
                .perform(
                    post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(propertyDTO))
                )
    ...
    void createPropertyWithExistingId() throws Exception {
        ...
        restPropertyMockMvc
            .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(propertyDTO)))
    ...
    void putWithMissingIdPathParamProperty() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPropertyMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(propertyDTO)))
    ...
    void patchWithMissingIdPathParamProperty() throws Exception {
        ...
        restPropertyMockMvc
            .perform(
                patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(propertyDTO))
            )
            .andExpect(status().isMethodNotAllowed());

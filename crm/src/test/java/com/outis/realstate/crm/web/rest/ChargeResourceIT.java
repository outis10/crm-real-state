...
import static com.outis.realstate.crm.domain.ChargeAsserts.*;
...
import com.outis.realstate.crm.repository.ChargeRepository;
import com.outis.realstate.crm.service.dto.ChargeDTO;
import com.outis.realstate.crm.service.mapper.ChargeMapper;
...
class ChargeResourceIT {
    ...
    public static Charge createUpdatedEntity(EntityManager em) {
        Charge updatedCharge = new Charge()
            .type(UPDATED_TYPE)
        ...
        Rental rental;
        if (TestUtil.findAll(em, Rental.class).isEmpty()) {
            rental = RentalResourceIT.createUpdatedEntity();
            em.persist(rental);
            em.flush();
        } else {
            rental = TestUtil.findAll(em, Rental.class).get(0);
        }
        updatedCharge.setRental(rental);
    ...
    void createCharge() throws Exception {
        ...
        var returnedChargeDTO = om.readValue(
            restChargeMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(chargeDTO)))
    ...
    void putWithMissingIdPathParamCharge() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restChargeMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(chargeDTO)))
    ...
    void partialUpdateChargeWithPatch() throws Exception {
        ...
        partialUpdatedCharge.type(UPDATED_TYPE).dueDate(UPDATED_DUE_DATE);
    ...
    void patchWithMissingIdPathParamCharge() throws Exception {
        ...
        restChargeMockMvc
            .perform(
                patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(chargeDTO))
            )
            .andExpect(status().isMethodNotAllowed());

...
import static com.outis.realstate.crm.domain.QuotationAsserts.*;
...
import com.outis.realstate.crm.repository.QuotationRepository;
import com.outis.realstate.crm.service.dto.QuotationDTO;
import com.outis.realstate.crm.service.mapper.QuotationMapper;
...
class QuotationResourceIT {
    ...
    void createQuotation() throws Exception {
        ...
        var returnedQuotationDTO = om.readValue(
            restQuotationMockMvc
                .perform(
                    post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(quotationDTO))
                )
    ...
    void createQuotationWithExistingId() throws Exception {
        ...
        restQuotationMockMvc
            .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(quotationDTO)))
    ...
    void putWithMissingIdPathParamQuotation() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restQuotationMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(quotationDTO)))
    ...
    void partialUpdateQuotationWithPatch() throws Exception {
        ...
        partialUpdatedQuotation.finalPrice(UPDATED_FINAL_PRICE).comments(UPDATED_COMMENTS).createdBy(UPDATED_CREATED_BY);
    ...
    void patchWithMissingIdPathParamQuotation() throws Exception {
        ...
        restQuotationMockMvc
            .perform(
                patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(quotationDTO))
            )
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteQuotation() throws Exception {
        ...
        restQuotationMockMvc
            .perform(delete(ENTITY_API_URL_ID, quotation.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
